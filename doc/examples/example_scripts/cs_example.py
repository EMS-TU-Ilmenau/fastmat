...
N = 100  # Problem size
M = int(N / compression_factor)  # Number of observations (specified over compression factor)
k = 3  # sparsity level
noise_power_db = -3

# Ground truth of scenario
ground_truth = np.zeros((N, ))
ground_truth[np.random.choice(N, k)] = np.random.randn(k)
...
x = x_clean + 10**(noise_power_db / 10.) * np.sqrt(1 / N) * (
...
true_defect_positions = np.argsort(np.abs(ground_truth), axis=0)[-k:][:, np.newaxis]
...
for index, m_phase_transition in enumerate(M_phase_transition):
    for _ in range(trials):
        ...
        b = Phi_pt * x
        recovered_support = np.argsort(np.abs(alg_omp.process(b, numMaxSteps=k)), axis=0)[-k:]
